    ...
    <div class="col-xl-12 content-box ">
        <div class="row row-featured row-featured-category">
            <div class="col-xl-12  box-title no-border">
                <div class="inner"><h2><span>{{ __('home.browse_by') }}</span>
            ...
            <div class="col-xl-2 col-md-3 col-sm-3 col-xs-4 f-category">
                ...
                <a href="{{ route('jobs.category', $category->id) }}"><img src="{{ asset('storage') . '/' . $category->image }}" class="img-responsive"
                    alt="img" height="auto" width="auto">
            ...
            <div class="content-box col-xl-12">
                <div class="row row-featured row-featured-category">
                    <div class="col-xl-12  box-title no-border">
                        <div class="inner">
                            <h2><span>{{ __('home.latest') }}</span> {{ __('home.job') }} 
                                <a href="{{ route('jobs.index') }}" class="sell-your-item"> {{ __('home.view_more') }} <i
                                        class="  icon-th-list"></i> </a></h2>
                        </div>
                    </div>
                        ...
                        <div class="item-list job-item">
                            <div class="row">
                                ...
                                <div class="col-sm-1 col-2  col-xs-2 no-padding photobox">
                                    <div class="add-image">
                                        @if ($job->image)
                                                <a href="#"><img alt="company logo" src="{{ asset('storage') . '/' . $job->image }}" alt="Image" class="rounded" height="50px"
                                                    class="thumbnail no-margin"></a>
                                            @else
                                                <a href="#"><img alt="company logo" src="{{ asset('storage') . '/' . $job->category->image }}"
                                                    class="thumbnail no-margin"></a>
                                            @endif
                                    </div>            
                                </div>
                                ...
                                <div class="col-sm-10 col-10  col-xs-10  add-desc-box">
                                    <div class="ads-details jobs-item">
                                        ...
                                        <h4 class="job-title"><a href="{{ route('jobs.show', $job->id) }}"> {{ $job->title }}</a>
                                        </h4>
                                        <span class="info-row"> <span class="item-location"><i
                                                    class="fa fa-map-marker-alt"></i> {{ $job->city->name }} </span>
                                            <span class="date"><i class=" icon-clock">
                                                </i>{{ $job->jobtype->name}}</span><span class=" salary"> <i
                                                    class=" icon-money"> </i>XAF{{ number_format($job->salary,0,",",".") }} a
                    ...
                    <div class="tab-box  save-search-bar text-center"><a class="text-uppercase"
                            href="{{ route('jobs.index') }}"> <i class=" icon-briefcase "></i> {{ __('home.view_all_job') }} </a></div>
                </div>
                ...
                <div class="inner-box">
                    ...
                    <div class="inner-box-content">
                        <ul class="cat-list arrow">
                            @foreach ($categories as $category)
                            <li><a href="job-list.html">{{ $category->name }} <span class="count">{{ $category->jobs->count() }}</span>
                            </a>
                            </li>
                            @endforeach
                        </ul>
